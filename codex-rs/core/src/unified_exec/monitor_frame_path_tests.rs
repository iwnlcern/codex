...
#[tokio::test]
async fn stale_armed_deadline_defers_to_new_producer_activity() {
    fixture(async {
        let (_, _, model) = hooked_monitor(
            PipelineHooks::default(),
            "python3 -c 'import os,time; os.write(1, b\"x\"*4097+b\"\\n\"); time.sleep(1.5); os.write(1, b\"y\"*4097+b\"\\n\"); time.sleep(30)'",
        )
        .await;
        let probe = probe();
        let quiet_interval = Duration::from_secs(2);
        let notice_budget = Duration::from_secs(5);
        wait_for(|| loss_counts(&probe)[0] == 1 && probe.armed_timer_origin().is_some()).await;
        let first = probe.producer_activity_at.lock().unwrap().unwrap();
        wait_for(|| probe.armed_timer_origin() == Some(first)).await;
        probe.freeze_refresh.store(true, Ordering::Release);
        wait_for(|| loss_counts(&probe)[0] == 2).await;
        let second = probe.producer_activity_at.lock().unwrap().unwrap();
        assert!(second > first);
        assert!(second < first + quiet_interval, "second producer activity precedes old expiry");
        assert_eq!(probe.armed_timer_origin(), Some(first));
        loop {
            let emitted = *probe.silence_notice_emitted_at.lock().unwrap();
            if let Some(emitted) = emitted {
                assert!(emitted >= second + quiet_interval,
                    "stale expiry emitted before two seconds of current producer silence");
            }
            if probe.armed_timer_origin() == Some(second) {
                break;
            }
            assert!(std::time::Instant::now() <= second + quiet_interval + notice_budget,
                "expiry revalidation did not re-arm within producer-relative budget");
            sleep(Duration::from_millis(10)).await;
        }
        assert_eq!(loss_counts(&probe)[0], 2, "deferred expiry must preserve both losses");
        assert!(probe.silence_notice_emitted_at.lock().unwrap().is_none());
        assert!(!model_text(&model).contains("MONITOR-NOTICE: loss"));
        probe.freeze_refresh.store(false, Ordering::Release);
        let deadline = tokio::time::Instant::from_std(second + quiet_interval + notice_budget);
        let text = timeout_at(deadline, async {
            loop {
                let text = model_text(&model);
                if text.contains("MONITOR-NOTICE: loss oversize records=2;") {
                    break text;
                }
                sleep(Duration::from_millis(10)).await;
            }
        }).await.expect("one loss notice within retained producer-relative budget");
        let emitted = probe.silence_notice_emitted_at.lock().unwrap().unwrap();
        assert!(emitted >= second + quiet_interval);
        assert!(emitted <= second + quiet_interval + notice_budget);
        assert_eq!(probe.silence_timer_origin(), Some(second));
        assert_eq!(text.matches("MONITOR-NOTICE: loss").count(), 1);
        assert_eq!(loss_counts(&probe)[0], 0);
        assert_eq!(*probe.producer_activity_at.lock().unwrap(), Some(second));
    }).await;
}
