...
impl MonitorManager {
    ...
    async fn take_for_cleanup(&self, id: &str, explicit_stop: bool) -> Option<JoinHandle<i32>> {
    ...
    fn spawn_cleanup(
        ...
        tokio::spawn(async move {
            ...
            if explicit_stop || shutdown.is_cancelled() {
                if let Some(process) = &record.process {
                    // Signal the same exit path as child termination before
                    // joining delivery, so its grace and final drain can run.
                    process.cancellation.cancel();
                    process.process.terminate();
                }
            }
            if let Some(mut task) = record.task.take() {
                // Self-deregister returns before this join. Shutdown discards
                // pipeline work even if an explicit stop is currently draining.
                tokio::select! {
                    biased;
                    ...
                    _ = &mut task => {}
                    _ = tokio::time::sleep(TRAILING_OUTPUT_GRACE + Duration::from_secs(5)), if explicit_stop => {
                        task.abort();
                        let _ = task.await;
                        let dropped_records = record.pipeline.as_ref().zip(record.process.as_ref())
                            .map(|(pipeline, process)| pipeline.ledger.for_attempt(process.committed).take());
                        tracing::warn!(monitor_id = %id, ?dropped_records,
                            "monitor stop delivery deadline expired; unflushed rows and notices discarded");
                    }
                }
            }
    ...
    pub(crate) async fn remove(&self, id: &str) -> Option<i32> {
        ...
        // The bounded delivery join above is separate from quiescence of
        // already-owned admissions. They settle before this stop returns even
        // if delivery exceeded its deadline and had to be aborted.
    ...
    pub(crate) async fn abort_all(&self) {
        // This lock is also held while the gate creates an owned admission:
        // setting stopped is the cutoff after which no new admission can enter.
        // Pipeline work is discarded; admissions owned before the cutoff settle
        // through the tracker below before shutdown returns.
        *self
            .stopped
            .lock()
        ...
        }
        {
            ...
            for (id, record) in records.drain() {
                let token = self.cleanups.token();
                Self::spawn_cleanup(id, record, token, self.shutdown.clone(), false);
            }
...
pub(crate) struct PipelinePathProbe {
    ...
    pub(crate) freeze_refresh: std::sync::atomic::AtomicBool,
...
struct ProducerObservation {
    ...
    armed_timer_origin: Option<std::time::Instant>,
}
#[cfg(test)]
impl PipelinePathProbe {
    pub(crate) fn armed_timer_origin(&self) -> Option<std::time::Instant> {
        self.observation.lock().unwrap().armed_timer_origin
    }
...
impl MonitorPipeline {
    ...
    fn build(#[cfg(test)] hooks: PipelineHooks) -> (Self, mpsc::Sender<TaggedChunk>) {
        ...
        let path_probe = Arc::new(PipelinePathProbe {
            ...
            freeze_refresh: std::sync::atomic::AtomicBool::new(false),
...
async fn delivery_loop(
    ...
    loop {
            ...
            loss_at = counters
                ...
                .then(|| {
                    ...
                    #[cfg(test)]
                    {
                        ...
                        path_probe.observation.lock().unwrap().armed_timer_origin = Some(activity);
                    }
        ...
        tokio::select! {
            ...
            _ = activity_check.tick(), if refresh => {}
            record = received => {
            ...
            () = wait_until(loss_at) => {
                let Some(losses) = counters.take_if_quiet(std::time::Instant::now(), Duration::from_secs(2)) else {
                    let activity = *counters
                        .last_activity
                        .lock()
                        .unwrap_or_else(std::sync::PoisonError::into_inner);
                    loss_at = Some(Instant::from_std(activity + Duration::from_secs(2)));
                    #[cfg(test)]
                    {
                        selected_origin = Some(activity);
                        path_probe.observation.lock().unwrap().armed_timer_origin = Some(activity);
                ...
                notices.extend(losses.into_iter().map(|(reason, count)| Notice::loss(reason, count)));
                flush(&session, &id, &description, &mut number, &mut pending, &mut notices).await;
                flush_at = None;
                loss_at = None;
            }
