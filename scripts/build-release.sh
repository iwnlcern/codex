...
build_macos() (
  ...
  lock_result=$(python3 scripts/lockfile-delta.py "$before_file" codex-rs/Cargo.lock codex-rs/Cargo.toml)
  printf '%s\n' "$lock_result"
...
lock_result=$(python3 /build/scripts/lockfile-delta.py /src/codex-rs/Cargo.lock /build/codex-rs/Cargo.lock /build/codex-rs/Cargo.toml)
printf '%s\n' "$lock_result"
[[ $lock_result =~ ^clean\ sha256=([0-9a-f]{64})$ ]]
lockfile_sha256=${BASH_REMATCH[1]}
