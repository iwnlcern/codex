...
impl PoolFixture {
    async fn new(approval: AskForApproval) -> Self {
        Self::with_model_server(approval, None).await
    }
    async fn with_model_server(
...
impl PoolFixture {
    async fn plan_mode(&self) {
        let mut collaboration_mode = self.session.collaboration_mode().await;
        collaboration_mode.mode = codex_protocol::config_types::ModeKind::Plan;
        self.session
            ...
            .await
            .expect("plan-mode idle admission records without starting a turn");
    }

    async fn held_pipeline(
        &self,
        command: &str,
    ) -> (String, Arc<PipelinePathProbe>, Arc<tokio::sync::Notify>) {
        let (request, context, mut pipeline) = self.request(command);
        let release = Arc::new(tokio::sync::Notify::new());
        pipeline.delivery_gate = Some(Arc::clone(&release));
        let probe = pipeline.path_probe();
        let id = bounded(self.session.services.monitor_manager.start_with_pipeline(
            &self.session,
            &context,
            request,
            "held pipeline".into(),
        ...
        .await
        .expect("real child monitor");
        (id, probe, release)
    }

    async fn recorded_text(&self) -> String {
        let history = self.session.clone_history().await;
        serde_json::to_string(&history.raw_items().collect::<Vec<_>>())
            .expect("actual session history")
    }
}

// The oversize line follows the complete row on the same real stdout stream:
// observing its pending loss proves the preceding row has already been framed.
async fn pending_pipeline_loss(probe: &PipelinePathProbe) {
    bounded(async {
        loop {
            let committed = probe.observation.lock().unwrap().committed;
            if committed.is_some_and(|attempt| {
                probe
                    .ledger
                    .for_attempt(attempt)
                    .gap_open
...
#[tokio::test]
async fn explicit_stop_drains_framed_row_and_pending_loss_notice_exactly_once() {
    let f = PoolFixture::new(AskForApproval::Never).await;
    f.plan_mode().await;
    let (id, probe, release) = f.held_pipeline(
        "printf 'STOP_FRAMED_ROW\n'; head -c 20000 /dev/zero | tr '\\0' x; printf '\n'; sleep 60",
    ).await;
    probe.hold_receive_for_final_loss();
    pending_pipeline_loss(&probe).await;
    assert_eq!(
        f.session.services.monitor_manager.slots.available_permits(),
        7
    );
    let before = f.recorded_text().await;
    assert!(!before.contains("STOP_FRAMED_ROW"));
    assert!(!before.contains("MONITOR-NOTICE: loss"));
    probe.resume_final_loss();
    release.notify_one();
    bounded(f.session.services.monitor_manager.remove(&id))
        .await
    ...
    let history = f.recorded_text().await;
    bounded(f.session.services.monitor_manager.abort_all()).await;
    assert_eq!(history.matches("STOP_FRAMED_ROW").count(), 1);
    assert_eq!(
        history
            .matches("MONITOR-NOTICE: loss oversize records=1;")
            .count(),
        1
    );
    assert_eq!(history.matches("MONITOR-NOTICE: exit").count(), 1);
}

#[tokio::test]
async fn shutdown_settles_owned_admission_and_drops_unadmitted_pipeline_work() {
    let server = core_test_support::responses::start_mock_server().await;
    let f = PoolFixture::with_model_server(AskForApproval::Never, Some(&server)).await;
    f.plan_mode().await;
    let (_held_id, probe, _held_release) = f.held_pipeline(
        "printf 'SHUTDOWN_UNADMITTED_ROW\n'; head -c 20000 /dev/zero | tr '\\0' x; printf '\n'; sleep 60",
    ).await;
    pending_pipeline_loss(&probe).await;
    let before = Arc::new(tokio::sync::Barrier::new(2));
    f.session
        .services
        .monitor_manager
        .install_gate_hooks(crate::session::GateHooks {
            before_start: Some(Arc::clone(&before)),
            after_start: None,
        });
    let (_owned_id, _owned_probe, owned_release) = f
        .held_pipeline("printf 'SHUTDOWN_OWNED_ROW\n'; sleep 60")
        .await;
    owned_release.notify_one();
    bounded(before.wait()).await;
    assert_eq!(
        f.session.services.monitor_manager.slots.available_permits(),
        6
    );
    ...
    // Reach the real gate after its cutoff; it must not create another admission.
    f.session
        .services
        .monitor_manager
        .deliver(
            ...
            ResponseItem::Message {
                ...
                content: vec![codex_protocol::models::ContentItem::InputText {
                    text: "SHUTDOWN_AFTER_CUTOFF".into(),
                }],
                phase: None,
        ...
        .await;
    let history = f.recorded_text().await;
    assert_eq!(history.matches("SHUTDOWN_OWNED_ROW").count(), 1);
    assert!(!history.contains("SHUTDOWN_UNADMITTED_ROW"));
    assert!(!history.contains("MONITOR-NOTICE: loss"));
    assert!(!history.contains("SHUTDOWN_AFTER_CUTOFF"));
    tokio::time::sleep(Duration::from_millis(1100)).await;
    assert_eq!(
        f.recorded_text().await,
        history,
        "no history work remains after shutdown"
    );
    assert!(
        server.received_requests().await.unwrap().is_empty(),
        "no model request after shutdown"
    );
    while let Ok(event) = f.events.try_recv() {
        assert!(
            !matches!(event.msg, EventMsg::TurnStarted(_)),
            "shutdown starts no model turn"
        );
    }
}
