...
The independent 72-name inventory is embedded in the script.
...
JUnit is copied before `just test -p codex-features --retries 0` overwrites it, and must contain exactly those 72 identities with no failures, errors, skips, or duplicates.
...
PR bodies begin with `state`, the peeled `upstream` SHA, `monitor-commit`, and `tested-head`, followed by an artifact link, a bounded diff stat, and exact commit and compare links.
A closed candidate is a no-op in discover, resume, and regenerate; reconcile refuses it with exit 1.
...
`resume` requires an open candidate, fetches and tests its current head only, rechecks the PR head after testing, and updates only the body using the current head's trailer for monitor provenance.
...
Every four hours the workflow schedules separate discovery and resume invocations; dispatch selects discovery, resume, regeneration, or reconcile through the same production `transition` entry point.
...
A test failure retains the one patch commit and links its evidence, with `tested-head: none`.
...
PR bodies contain the four ordered headers, the test-artifact locator, `git diff --stat <peeled tag>..<head>`, and links to `<server>/<repo>/commit/<head>` and `<server>/<repo>/compare/<tag>...<head>`.
Every published candidate is exactly one commit above the peeled tag, including report-only conflicts, so the commit view is the exact candidate diff.
The body is composed and measured before any remote write; 60,000 or more characters exits 1 before the label or ref is written.
The header is `tested-head: none` for `conflict` and `tests-failed`; only a clean receipt names its tested SHA.
Preparation writes a `Monitor-Commit: <sha>` trailer into clean and report-only candidate commits.
Resume and regeneration derive provenance from exactly one valid trailer in the head, never from the PR body; a missing, malformed, or duplicate trailer stops before tests or remote writes.
When repairing a report-only candidate, preserve the originating trailer in the new single commit above the tag.

Discovery creates the label idempotently before pushing, so a failed label command writes no candidate ref.
A failed `gh pr create` is indeterminate and never causes rollback in the same run.
The script rereads candidate state once: an open PR at its pushed head is accepted and its body corrected only if different from this run's prepared body; an open PR at another head is unresolved; an observed absence is indeterminate; a closed, ambiguous, or failed read is unknown.
The latter three outcomes exit 1 and retain `publication-hold.json` under the attempt's `MONITOR_REBASE_RESULTS` root, recording tag, pushed head, outcome, command output, and timed read evidence.
A subsequent discover refuses the absent-destination lease if the orphan ref remains; it skips an open labeled PR, and skips a closed labeled PR forever.
The failed run and its hold record remain the evidence for investigation.

Reconcile is a fifth explicit, dispatch-only mode and is never scheduled.
Invoke it only after the operator judges the indeterminate window closed, copying the expected head from the retained hold record rather than a fresh remote read:

```bash
bash scripts/rebase-monitor.sh transition reconcile --tag rust-v0.154.0 --expected-head <pushed-head-from-publication-hold.json>
```

For workflow dispatch select `reconcile`, set `tag`, and copy that SHA into `expected_head`.
The `expected_head` input is required for reconcile and regenerate and ignored for discover and resume.
Choose a fresh local `MONITOR_REBASE_RESULTS` root for every attempt; hosted runs already use distinct `runner.temp/<run id>/<run attempt>/<mode>` roots.
Reconcile records actual and expected heads and never prepares or tests.
With no labeled PR and a matching actual head, it deletes only with `--force-with-lease=refs/heads/rebase/<tag>:<expected-head>`, then confirms absence before succeeding; the next discovery can prepare again.
A foreign head, refused lease, failed delete, or unconfirmed absence exits 1 with only the values actually read; unread values remain explicitly unread.
With an open PR at the expected head, reconcile validates its label, head, four headers, peeled upstream, trailer provenance, and state-consistent tested-head, while preserving its body and artifact locator byte for byte.
A valid delayed publication reports `publication observed` without editing; malformed or stale bodies fail and require ordinary `resume` to produce fresh test evidence before editing.
Open PRs at other heads and closed candidates are refused without mutation.
The accepted residual race is that a create request not materialized by this later human-initiated reconcile is treated as failed; there is no transaction across Git refs and the PR service.

If resume or regenerate cannot edit the PR, the run exits 1 and leaves the ref and immutable receipt intact.
Retry with ordinary resume and a fresh attempt-specific results root: it tests the current head again, publishes the new receipt, and takes monitor provenance from that head's trailer even if the old body names an earlier monitor commit.
An earlier receipt is never reused or relabeled, and a newly pushed head remains ineligible until its own clean receipt is published.
Proof refs outside `rebase/<tag>` never write a candidate PR body.
